...
def cmd_extract(a):
    ...
    todo = [f for f in files if a.force or not cache.is_fresh(con, f, a.sr, a.win, a.hop)]
...
def _pesos_ejec(a):
    return {"timing": a.w_timing, "groove": a.w_groove, "notas": a.w_notas,
...
def _load(a):
    ...
    df = scoring.score_windows(pd.DataFrame(rows), a.peso_sonido, a.peso_ejec,
                               a.modo, a.sonido_min, _pesos_ejec(a))
    return con, df, files
...
def cmd_rank(a):
    ...
    cols = ["rank", "Toma", "dur_min", "n_win", a.sort_by, "ejecucion", "timing",
            "groove", "notas", "afinacion", "creatividad", "desarrollo",
            "sonido", "best_start"]
    ...
    pe = _pesos_ejec(a)
    print(f"\nTop {a.top} de {len(d)} tomas | orden: {a.sort_by} | {modo}")
    print(f"pesos ejecución: " + " ".join(f"{k}={v}" for k, v in pe.items()) + "\n")
        ...
        for nombre in nombres:
            ...
            for _, row in hit.iterrows():
                ...
                print(f"  {marca} #{int(row.name)+1:4d}/{len(d)} pct={row['pct']:5.1f} "
                      f"{a.sort_by}={row[a.sort_by]:.3f} | tim={row['timing']:.2f} "
                      f"gro={row['groove']:.2f} afi={row['afinacion']:.2f} "
                      f"des={row['desarrollo']:.2f} son={row['sonido']:.2f}  {row['Toma']}")
...
def cmd_compilado(a):
    ...
    # 'desarrollo' es de archivo: no se le puede preguntar a 90 s si evoluciona.
    col = a.seg_by
    if col in ("desarrollo", "score_med", "score_best", "ejecucion"):
        col = "score"
    tramos, info = [], []
...
def main(argv=None):
    ...
    def weights(sp):
        ...
        sp.add_argument("--w-timing", type=float, default=0.25)
        sp.add_argument("--w-groove", type=float, default=0.20)
        sp.add_argument("--w-notas", type=float, default=0.15,
                        help="peso de 'notas correctas' (penaliza pifies)")
        sp.add_argument("--w-afinacion", type=float, default=0.10)
        sp.add_argument("--w-desarrollo", type=float, default=0.10)
    ...
    common(u)
    u.add_argument("--sim", type=float, default=0.9995)
    ...
    m.add_argument("--seg-by", default="score",
                   choices=["score", "timing", "groove", "notas", "afinacion", "sonido"],
                   help="criterio para elegir el tramo DENTRO de cada toma "
                        "(score = la mezcla ponderada; desarrollo no aplica a 90s)")
    ...
    for sp in (r, v, m):
        sp.add_argument("--sort-by", default="score_med",
                        choices=["score_med", "score_best", "ejecucion", "timing",
                                 "groove", "notas", "afinacion", "desarrollo",
                                 "creatividad", "sonido"],
                        help="dimensión por la que ordenar el ranking")
